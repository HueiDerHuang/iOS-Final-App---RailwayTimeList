...
class RecordTableViewController: UITableViewController {
    ...
    var recordCellIdentifier = "recordCellIdentifier"
    ...
    override func viewDidLoad() {
        ...
        if let recordLists = RecordList.readFromFile() {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.originStationLabel.text = recordLists[indexPath.row].OriginStation
        cell.destinationStationLabel.text = recordLists[indexPath.row].DestinationStation
        cell.depTimeLabel.text = recordLists[indexPath.row].DepTime
        cell.arrTimeLabel.text = recordLists[indexPath.row].ArrTime
        cell.noteLabel.text = recordLists[indexPath.row].Note
        cell.dateLabel.text = recordLists[indexPath.row].Date
        cell.trainInfoLabel.text = recordLists[indexPath.row].CarClassAndTrain
        if recordLists[indexPath.row].CarClass == "區間車" || recordLists[indexPath.row].CarClass == "區間快" {
            cell.trainInfoLabel.backgroundColor = UIColor(red: 3/255, green: 121/255, blue: 251/255, alpha: 0.8)
        } else {
            cell.trainInfoLabel.backgroundColor = UIColor(red: 255/255, green: 147/255, blue: 0/255, alpha: 0.8)
        }
        var indexForImage = 0
        if recordLists[indexPath.row].BreastFeed == "Y" {
            cell.imageViews[indexForImage].image = breastFeedImage
            indexForImage += 1
        }
        if recordLists[indexPath.row].Cripple == "Y" {
            cell.imageViews[indexForImage].image = crippleImage

            indexForImage += 1
        }
        if recordLists[indexPath.row].OverNightStn == "Y" {
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            RecordList.saveToFile(recordLists: recordLists)
